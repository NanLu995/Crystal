...
namespace Server.Database
{
    public partial class MonsterInfoFormNew : Form
    {
        ...
        private void monsterInfoGridView_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "值必须是正整数positive integer";
            }

            if (col.ValueType == typeof(int) && !int.TryParse(val, out _))
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "值必须是整数integer";
            }
            else if (col.ValueType == typeof(byte) && !byte.TryParse(val, out _))
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "值必须是一个字节byte";
            }
            else if (col.ValueType == typeof(short) && !short.TryParse(val, out _))
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "值必须是-32768~32767短整型short";
            }
            else if (col.ValueType == typeof(ushort) && !ushort.TryParse(val, out _))
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "值必须是0~65535无符号整数ushort";
            }
            else if (col.ValueType == typeof(long) && !long.TryParse(val, out _))
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "值必须是长整型long";
            }
        }
                                ...
                                for (int j = 1; j < columns.Length; j++)
                                {
                                    ...
                                    if (dataColumn.Name == "MonsterName")
                                    {
                                        ...
                                        if (existingRow != null)
                                        {
                                            ...
                                            {
                                                throw new Exception($"怪物名称为 {cells[j]} 的项已存在.");
                                            }
                                        }
